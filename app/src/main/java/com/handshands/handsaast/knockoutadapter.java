...
public class knockoutadapter extends RecyclerView.Adapter<knockoutadapter.mh> {
    ArrayList<knockoutinheret>list;
    Context context;

    public knockoutadapter(ArrayList<knockoutinheret> list, Context context) {
        this.list = list;
    ...
    public mh onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(context).inflate(R.layout.rawknockout,parent,false);
        return new mh(v);
    }
...
holder.fname.setText(list.get(position).getFirstteam());
        ...
        holder.l2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent j=new Intent(context,information.class);
