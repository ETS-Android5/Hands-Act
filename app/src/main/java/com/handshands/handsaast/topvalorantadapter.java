...
public class topvalorantadapter extends RecyclerView.Adapter<topvalorantadapter.mh> {
    ArrayList<topvalorantmodel>list;
    Context context;

    public topvalorantadapter(ArrayList<topvalorantmodel> list, Context context) {
        this.list = list;
    ...
    @NonNull
    @Override
    public topvalorantadapter.mh onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(context).inflate(R.layout.rawtopinfo,parent,false);
        return new mh(view);
    }

    @Override
    public void onBindViewHolder(@NonNull topvalorantadapter.mh holder, int position) {
        holder.name.setText(list.get(position).getName());
        holder.pos.setText(list.get(position).getTeam());
